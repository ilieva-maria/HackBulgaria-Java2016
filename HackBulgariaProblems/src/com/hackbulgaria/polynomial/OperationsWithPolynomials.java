...
public class OperationsWithPolynomials {
	...
	public static Polynomial multiplyByConstant(Polynomial polynomial, int constant) {
		Polynomial result = new Polynomial();
	...
	public static Polynomial firstDerivative(Polynomial polynomial) {
		Polynomial result = new Polynomial();
		Iterator<Map.Entry<Integer, Integer>> it = polynomial.getPolynom().entrySet().iterator();
		while (it.hasNext()) {
			Map.Entry<Integer, Integer> pair = it.next();
	...
	public static long evaluate(Polynomial polynomial, int x) {
		int result = 0;
		Iterator<Map.Entry<Integer, Integer>> it = polynomial.getPolynom().entrySet().iterator();
		while (it.hasNext()) {
			Map.Entry<Integer, Integer> pair = it.next();
